  ...
  return (
    <section className="cart">
      <div className="cart__header">
        ...
        {cartContents.length > 0 ? (<button className="cart__remove" onClick={handleRemoveAll} >Remove all</button>) : <button className="cart__remove cart__remove--empty" >{""}</button>}
      </div>
      ...
      {cartContents.length > 0 ? (
          ...
          <li className="cart__content-item cart__content-item--empty">There are no items in your cart</li>
        )}
      </ul>
      <div className="cart__footer">
      {cartContents.length > 0 ? ( <h5 className="cart__total">{`Total: $${calcTotal()}`}</h5>) : <h5 className="cart__total cart___total--empty">{""}</h5>}
      </div>
    </section>
  );
};
